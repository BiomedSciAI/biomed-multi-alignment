# scRNA based cell type Annotation Prediction with MAMMAL
...
Also included are scripts to build an AnnData from the Zheng68k data (see below) that can be used for the fine tune, and a script to prepare a standard scRNA-sec AnnData file for Mammal.
...
The [data/process_h5ad_data.py](data/process_h5ad_data.py) script runs the data preprocessing as described above.  This process consists of
 ...
 2. normalizing the total counts for all the reads of the cell to 1.0
 3. passing the counts through `log(value+1)` which shifts the counts to the range of zero to one.
...
This script is used to convert from a `data.h5ad` file to a `data_processed.h5ad` file, containing the cells that passed the filters and the binned data.

 Note that the details of the preprocessing in the script are ones we found to work well for this task on this and similar data.  For different data accusation process the preprocessing may require changes. The parameters of the preprocessing in the script can be changed via command line arguments.
...
The script includes instructions and code for downloading, processing and packing the [Zheng68k](TODO:link) dataset.

Note that the AnnData generated by this script is a standard scRNA-sec AnnData, and needs to be preprocessed into bins as described above.
...
4. Go into the data directory and run the **data preparation** script
    ...
    Use `python Zheng68k_to_anndata.py --help` to see the possible command line options which control this process.
