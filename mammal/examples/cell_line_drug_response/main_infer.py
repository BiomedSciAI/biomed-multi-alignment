...
def sort_genes_by_value_and_name(
    genes,
    expressions
):
    """
...
def cell_line_drug_infer(
    ...
    sample_dict = dict()
    sample_dict[ENCODER_INPUTS_STR] = f"<@TOKENIZER-TYPE=SMILES><MASK>" \
    + f"<@TOKENIZER-TYPE=SMILES><MOLECULAR_ENTITY><MOLECULAR_ENTITY_SMALL_MOLECULE><SMILES_SEQUENCE>{drug_smiles}" \
    + f"<@TOKENIZER-TYPE=GENE><MOLECULAR_ENTITY><MOLECULAR_ENTITY_CELL_GENE_EXPRESSION_RANKED>"\
    + "".join(gene_seq_formatted)+"<EOS>"
    ...
    if SCALARS_PREDICTION_HEAD_LOGITS in batch_dict:
        ...
        print(f"WARNING: No prediction found")
        return None
